...
Optionen:
  --branch <branch>          Branch fuer den Download von install.sh setzen
  --force-license-prompts   Lizenzabfragen auch bei vorhandenen Tools anzeigen
  --help                     Hilfe anzeigen

Alle weiteren Parameter werden unveraendert an install.sh weitergegeben.
Welche Parameter gueltig sind, entscheidet ausschliesslich install.sh.
  -h, --help            Hilfe anzeigen
EOF
}

SELECTED_BRANCH=""
USER_PROVIDED_BRANCH=false
ORIGINAL_ARGS=("$@")
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --branch)
      ...
      USER_PROVIDED_BRANCH=true
      SELECTED_BRANCH="$2"
      shift 2
      ;;
    --branch=*)
      USER_PROVIDED_BRANCH=true
      SELECTED_BRANCH="${1#--branch=}"
      if [[ -z "$SELECTED_BRANCH" ]]; then
        echo "Fehlender Wert fuer --branch" >&2
        exit 1
      fi
      shift
      ;;
    -h|--help)
      ...
      ;;
    *)
      # setup.sh wertet nur --branch aus; alles andere verarbeitet install.sh
      shift
      ;;
  esac
done
...
if [[ $EUID -eq 0 ]]; then
  if [[ "$USER_PROVIDED_BRANCH" == true ]]; then
    bash "$INSTALL_SCRIPT" "${ORIGINAL_ARGS[@]}"
  else
    bash "$INSTALL_SCRIPT" "${ORIGINAL_ARGS[@]}" --branch "$SELECTED_BRANCH"
  fi
else
  ...
  if [[ "$USER_PROVIDED_BRANCH" == true ]]; then
    sudo bash "$INSTALL_SCRIPT" "${ORIGINAL_ARGS[@]}"
  else
    sudo bash "$INSTALL_SCRIPT" "${ORIGINAL_ARGS[@]}" --branch "$SELECTED_BRANCH"
  fi
fi
